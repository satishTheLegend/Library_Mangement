...
namespace Library_Mangement.Droid.Services
{
    public class MediaService : IMediaService
    {
        ...
        public async Task<IList<MediaAsset>> LoadMediaAsync()
        {
                    ...
                    if (cursor.Count > 0)
                    {
                        while (cursor.MoveToNext())
                        {
                            try
                            {
                                ...
                                var tempPath = System.IO.Path.Combine(tmpPath, $"upload_{name}");
                                var isFileAlreadySynced = await App.Database.Settings.IsDataContainsInValue("UP_Images", filePath);
                                if (isFileAlreadySynced)
                                    continue;
