...
namespace EDFSCore
{
    public class EDFSFunctionManager : NSUrlSessionDataDelegate, IEDFSCore
    {
#if __MAC__
        ...
        async Task<ApiResponseResult<T>> IEDFSCore.PostRequestAsync<T>(string ApiUrl, IDictionary<string, string> Parameters, IDictionary<string, string> Headers)
        {
            ApiResponseResult<object> errorResponse = new ApiResponseResult<object>();
            //T response = default(T);
            int statusCode = 0;
